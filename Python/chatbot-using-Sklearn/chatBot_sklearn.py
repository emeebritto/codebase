...
#get artical url
article= Article('https://pt.wikipedia.org/wiki/cafe')
article.download()
article.parse()
article.nlp()
corpus=article.text
...
def greeting(sentence):
  for word in sentence.split():
    if word.lower() in greeting_input:
      return random.choice(greeting_response)
flag=True
print("hello!!! this is Nexa (type bye to exit)")
while(flag==True):
    ...
    if(user_response=='thanks' or user_response=='thank you'):
      ...
      print("Nexa: anytime :)")
       ...
       if( greeting(user_response) != None):
         print("Nexa: "+ greeting(user_response))
       else:
         print("Nexa:"+response(user_response))
  else:
    flag=False
