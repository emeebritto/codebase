...
_settings_dir = os.path.abspath(
    os.path.join(
        _install_directory, os.pardir, os.pardir, "User", "Nexa.sublime-settings"
    )
)
_package_dir = os.path.abspath(
    os.path.join(_install_directory, os.pardir, os.pardir, "Nexa", "package.json")
)
...
def is_nexa_disabled(view):
    return view.settings().get("nexa-disabled", False)
